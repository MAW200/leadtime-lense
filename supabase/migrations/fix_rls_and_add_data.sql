...
-- 2. Create permissive policies for Development (Allow all authenticated users)
-- In a real production app with custom claims, we would check auth.jwt() ->> 'role'
-- But since the frontend RoleContext doesn't sync to JWT yet, we allow authenticated users to proceed.

CREATE POLICY "Allow authenticated full access on master_products" ON master_products
    FOR ALL USING (auth.role() = 'authenticated');

CREATE POLICY "Allow authenticated full access on inventory_items" ON inventory_items
    FOR ALL USING (auth.role() = 'authenticated');

-- 3. Insert Dummy Data

-- Master Product 1: Nippon Paint
DO $$
DECLARE
    master_id_1 UUID;
    master_id_2 UUID;
